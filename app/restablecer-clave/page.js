...
function ResetInner() {
  ...
  async function onSubmit(e) {
    ...
    try {
      ...
      if (res.ok) setDone(true);
        ...
        setError(j.error === 'invalid_or_expired' ? 'This reset link is invalid or has expired.' : 'Something went wrong. Try again.');
      }
    } catch {
  ...
  if (!token) {
    return <p className="text-[13px] text-ink/60">Missing reset token. Request a new link from the sign-in page.</p>;
  }

  if (done) {
    return (
      <>
        <h1 className="text-[19px] font-bold tracking-head mb-1">Password updated</h1>
        <p className="text-[13px] text-ink/60 leading-relaxed">Your password has been changed. You can now sign in with your new password.</p>
        <Link href="/login" className="block text-center w-full mt-5 px-4 py-3 rounded-pill bg-ink text-paper text-[14px] font-semibold shadow-hard-soft">
          Go to sign in
        </Link>
      </>
    );
  ...
  return (
    <form onSubmit={onSubmit}>
      <h1 className="text-[19px] font-bold tracking-head mb-1">Set a new password</h1>
      <p className="text-[13px] text-ink/55 mb-5">Choose a new password for your admin account.</p>
    ...
    <div className="min-h-screen bg-paper flex items-center justify-center px-5">
      <div className="w-full max-w-[400px]">
        <div className="text-center mb-7">
          ...
          <p className="mt-1 text-[12px] font-mono tracking-label uppercase text-ink/45">Reset password</p>
        </div>
