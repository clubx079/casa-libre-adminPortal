...
// Retry transient failures (429 / 5xx / timeout) with backoff — Wasi rate-limits
// bursty scrapes, and a bare failure here would fall back to a data-less record
// that then gets wrongly quarantined as no_price / no_contact.
async function fetchText(url, tries = 3) {
  ...
  for (let a = 0; a < tries; a++) {
    try {
      ...
      if (res.status === 429 || res.status >= 500) throw new Error(`Habitamia ${res.status} @ ${url}`);
      if (!res.ok) throw new Error(`Habitamia ${res.status} @ ${url}`);
      return res.text();
    } catch (e) {
      lastErr = e;
      if (a < tries - 1) await new Promise((r) => setTimeout(r, 1200 * (a + 1)));
    }
  }
  throw lastErr;
}
