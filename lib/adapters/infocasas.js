...
async function fetchPage(config, filters, skip, top) {
  ...
  for (let pg = firstPage; pg <= lastPage; pg++) {
    ...
    if (count < PER) { exhausted = true; break; }           // reached the end of this slice
  }
  ...
  // End-of-slice ONLY when the raw pages were exhausted. A window shortened purely
  // by the id-filter must NOT end the slice, or the backfill truncates and the
  // shard flips to incremental with its tail unscraped.
  const total = exhausted ? skip + windowItems.length : Number.MAX_SAFE_INTEGER;
