...
// coordinates + the full gallery + the agent contact card. Config-driven so any
// Raíces site works by base_url.
...
function parseDetail(html) {
  ...
  // Agent contact card: <section class="property_contact"> holds the seller's
  // name (h4.mediumTitle) + a telf:/phoneLink number. Scope extraction to that
  // section so the site-wide office WhatsApp button elsewhere is ignored.
  const contactSection = (main.match(/property_contact[\s\S]*/) || [null])[0] || main;
  const contactName = (contactSection.match(/mediumTitle">\s*([^<]+?)\s*<\/h4>/i) || [])[1] || null;
  ...
  return {
    ...
    description,
    images,
    contactName: contactName ? contactName.replace(/\s+/g, ' ').trim() : (contact && contact.name) || null,
