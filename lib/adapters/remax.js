...
function mapListing(content, { source, baseUrl, config, pygPerUsd }) {
  ...
  const imgs = (content.ListingImages || [])
    ...
    .slice(0, MAX_IMAGES)
    .map((im, i) => {
      // GryphTech serves the SAME photo watermark-free at /Large/ — the /LargeWM/
      // variant only adds the RE/MAX overlay. Always pull the clean copy so we
      // never have to inpaint a RE/MAX watermark (which the AI fill mangled).
      const folder = 'Large';
      return {
