...
import { badgeAttachments } from './emailBadges.js';
import { countryInfo } from './automationAdmin.js';
...
// ── Email templates: "Send test" from the admin editor ──────────────────────────
// Sends from the SAME address real automation emails use for that country
// (no-reply@<country domain>), with the app badges attached as cid images.

export const ADMIN_TEST_EMAIL = process.env.ADMIN_TEST_EMAIL || 'omar@airosofts.com';

export async function sendTemplateTest({ country, subject, html, text }) {
  if (!resend) return { ok: false, error: 'email_not_configured' };
  const from = `Casa Libre <no-reply@${countryInfo(country).sender}>`;
  try {
    const { data, error } = await withTimeout(
      resend.emails.send({ from, to: ADMIN_TEST_EMAIL, subject: `[TEST] ${subject}`, html, text, attachments: badgeAttachments() }),
      15000
    );
    if (error) return { ok: false, error: error.message || 'send_failed' };
    return { ok: true, id: data?.id || null, to: ADMIN_TEST_EMAIL };
  } catch (e) {
    return { ok: false, error: e?.message || 'send_failed' };
  }
}
