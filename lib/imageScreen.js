...
//   3. Google Cloud Vision      — LABEL + TEXT detection, then:
...
async function classifyOne(url, apiKey, counter) {
  try {
    ...
    if (counter) counter.vision++; // billed Vision request (2 features)
    const vr = await fetch(`${VISION_URL}?key=${apiKey}`, {
      ...
      body: JSON.stringify({
        requests: [{
          ...
          // Object Localization + Logo Detection dropped (cost saving). House
          // detection uses labels; TEXT drives the watermark/branding checks
          // below; logo-only images are still rejected via the label cues
          // ('logo'/'brand'/'trademark'/'watermark' in NON_HOUSE_CUES).
          features: [
    ...
    const all = (r0.labelAnnotations || []).map((l) => String(l.description).toLowerCase());
    ...
    const branded = hasPhone || hasUrl || heavyText || bigTextOverlay;
...
export async function screenPropertyImages(urls, source = 'unknown') {
  ...
  // Casa Libre Vision = 2 features/image (Label+Text; Logo + Object Localization dropped for cost).
  void logApiCall({ api: 'vision', source, path: 'imageScreen', calls: counter.vision * 2 });
