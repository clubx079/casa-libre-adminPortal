...
async function classifyOne(url, apiKey, counter) {
  try {
    ...
    if (counter) counter.vision++; // billed Vision request (3 features)
    const vr = await fetch(`${VISION_URL}?key=${apiKey}`, {
      ...
      body: JSON.stringify({
        requests: [{
          ...
          // Object Localization dropped (cost saving) — house detection uses
          // labels; Text + Logo drive the watermark/branding checks below.
          features: [
...
export async function screenPropertyImages(urls, source = 'unknown') {
  ...
  // Casa Libre Vision = 3 features/image (Label+Text+Logo; Object Localization dropped).
  void logApiCall({ api: 'vision', source, path: 'imageScreen', calls: counter.vision * 3 });
