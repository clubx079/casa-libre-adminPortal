...
async function classifyOne(url, apiKey) {
  try {
    ...
    if (!vr.ok) return { url, keep: true }; // pass on Vision error — never block a real photo on infra
    ...
    // Reject branded/watermarked photos — a stamped phone number, URL, or heavy
    // text overlay means a competing agency branded the image.
    ...
    if (hasPhone || hasUrl || heavyText) return { url, keep: false, reason: 'branded' };

    const hasHouse = all.some((d) => HOUSE_RE.test(d));
    if (hasHouse) return { url, keep: true };                    // any property cue → keep
    const clearlyNon = all.some((d) => NON_HOUSE_RE.test(d));
    if (clearlyNon) return { url, keep: false, reason: 'non_property', labels: all.slice(0, 5) };
    return { url, keep: true };                                   // ambiguous → keep (lenient)
  } catch {
    return { url, keep: true };                                   // pass through on any error
  }
}
...
// Public: given image URLs, return { kept: [...urls], rejected: [{url, reason}] }.
export async function screenPropertyImages(urls) {
  const list = (urls || []).filter(Boolean);
  if (!list.length) return { kept: [], rejected: [] };
  ...
  // Stage 3 — Vision (only if a key is configured)
  const apiKey = visionKey();
  if (!apiKey) return { kept: sizeKept, rejected };

  const visionResults = await batched(sizeKept, (u) => classifyOne(u, apiKey));
  const kept = [];
  visionResults.forEach((r) => {
    if (!r) return;                       // errored batch slot → drop silently (rare)
    if (r.keep) kept.push(r.url);
