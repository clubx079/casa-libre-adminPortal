...
// DB is passed in per call (country-aware): isDuplicate(key, db) / quarantine({…, db}).

// Reason-code → label map lives in ./ingestLabels (plain module) so the client
// review page can import the labels without pulling in this server-only file.
export { REASON_LABELS } from './ingestLabels';
...
export function dedupeKey(row, zoneCanonical) {
  ...
  // Type is only split land vs building — the AI can label the SAME unit "Casa" on
  // one source and "Condominio" on another, so the specific building type must NOT
  // be part of the fingerprint or real duplicates slip through.
...
export async function isDuplicate(key, db) {
  if (!key) return null;
  const { select } = db;
...
export function validateListing(row, rate) {
  ...
  if (rent) {
    ...
    // Upper bound: no real Paraguay listing exceeds ~$50M. Absurd values are
    // seller data-entry errors (extra zeros, e.g. $18,500 typed as 18500000000)
    // that source sites store verbatim — quarantine instead of publishing.
    else if (Number(usd) > 50000000) reasons.push('price_above_ceiling');
  }
...
export async function quarantine({ sourceId, externalId, row, reasons, dedupeKey: dk, duplicateOf, db }) {
  const { insert } = db;
