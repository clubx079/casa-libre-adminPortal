...
export async function runJob({ runId }) {
  ...
  const s = { found: 0, inserted: 0, updated: 0, skipped: 0, images: 0, imagesRejected: 0, quarantined: 0, duplicates: 0, target: limit, page: 0 };
  const recent = [];
  const errors = [];
    ...
    while (s.found < limit && skip < total) {
      ...
      for (const m of mapped) {
        ...
        try {
          ...
          let imgUploaded = 0;
          let featureUrl = null;
          let screened = m.images;
          if (m.images.length) {
            const { kept, rejected } = await screenPropertyImages(m.images.map((i) => i.source_url));
            const keptSet = new Set(kept);
            screened = m.images.filter((i) => keptSet.has(i.source_url));
            if (rejected.length) {
              s.imagesRejected = (s.imagesRejected || 0) + rejected.length;
              pushRecent({ kind: 'item', action: 'img_screen', id: m.external_id, rejected: rejected.length, kept: screened.length, reasons: [...new Set(rejected.map((r) => r.reason))] });
            }
            if (screened.length) {
