...
import { normalizeZone, dedupeKey, isDuplicate, validateListing, imageStatus, quarantine } from './ingest';
...
export async function runJob({ runId }) {
  ...
  const s = { found: 0, inserted: 0, updated: 0, skipped: 0, images: 0, quarantined: 0, duplicates: 0, target: limit, page: 0 };
  const recent = [];
  const errors = [];
    ...
    while (s.found < limit && skip < total) {
      ...
      for (const m of mapped) {
        ...
        try {
          ...
          let featureUrl = null;
          if (m.images.length) {
            const r = await mirrorImages(source, m.external_id, propertyId, m.images);
            imgUploaded = r.uploaded; s.images += r.uploaded; featureUrl = r.featureUrl;
          }
          // #28 Image screening — ok | broken (had URLs, none mirrored) | none.
          const imgPatch = { image_status: imageStatus(m.images.length, featureUrl) };
