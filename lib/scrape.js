...
export async function runJob({ runId }) {
  ...
  const s = { found: 0, inserted: 0, updated: 0, skipped: 0, images: 0, target: limit, page: 0 };
  const recent = [];
  const errors = [];
    ...
    while (s.found < limit && skip < total) {
      ...
      for (const m of mapped) {
        ...
        try {
          ...
          let imgUploaded = 0;
          if (m.images.length) {
            const { uploaded, featureUrl } = await mirrorImages(source, m.external_id, propertyId, m.images);
            imgUploaded = uploaded; s.images += uploaded;
            if (featureUrl) await update('properties', `id=eq.${propertyId}`, { feature_image_url: featureUrl }, { returning: 'minimal' });
          }
