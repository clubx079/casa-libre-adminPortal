...
import { activeCountry } from './adminCountry';
...
import { makeCostBreaker } from './costBreaker';
...
import skyone from './adapters/skyone';
import remaxbo from './adapters/remaxbo';
import remaxuy from './adapters/remaxuy';
import coldwell from './adapters/coldwell';
import { genShortCode } from './shortcode';
...
const ADAPTERS = {
  ...
  skyone_api: skyone,
  remax_bo_api: remaxbo,
  remax_uy_api: remaxuy,
  coldwell_html: coldwell,
};
const PAGE = 50;
const RECENT_MAX = 40;
const FLUSH_MS = 700;
...
export async function getSourceByKey(key, country) {
  const { select } = dbFor(country || activeCountry());
...
async function readControl(db, runId) {
  const { select } = db;
...
async function reap(db, id, status = 'failed') {
  const { update } = db;
...
export async function getActiveRun(sourceId, country) {
  const db = dbFor(country || activeCountry());
  const { select } = db;
  ...
  let alive = null;
  for (const r of rows) {
    if (runIsStale(r)) await reap(db, r.id);
...
export async function requestControl(runId, control, country) {
  const db = dbFor(country || activeCountry());
...
async function mirrorImages(db, source, externalId, propertyId, images) {
  const { insert } = db;
  let uploaded = 0;
  const rows = [];
...
async function stampListingImages(db, propertyId) {
  const { select, update } = db;
...
export async function startRun({ sourceKey, filters = {}, trigger = 'manual', country }) {
  const country_ = country || activeCountry();
  const { insert } = dbFor(country_);
  const source = await getSourceByKey(sourceKey, country_);
  const active = await getActiveRun(source.id, country_); // reaps stale runs, returns a live one or null
  if (active) return { runId: active.id, source, attached: true, country: country_ };
  ...
  return { runId: run.id, source, attached: false, country: country_ };
}

// The background job. Loads the run + source, works, flushes progress, obeys control.
export async function runJob({ runId, country }) {
  const cc = country || activeCountry();
  const db = dbFor(cc);
  const { select, insert, update, remove } = db;
  const [run] = await select('scrape_runs', `id=eq.${runId}&limit=1`);
  if (!run) throw new Error('Run not found');
  const source = await getSourceById(db, run.source_id);
  ...
  const gate = async () => {
    let c = await readControl(db, runId);
    if (c === 'pause') {
      await flush(true, 'paused');
      while ((c = await readControl(db, runId)) === 'pause') {
        await sleep(1500);
  ...
  const breaker = makeCostBreaker(source.key, select); // in-run cost breaker (runaway cap + mid-run kill switch)
  try {
    ...
    while (s.found < limit && skip < total) {
      ...
      for (const m of mapped) {
        if ((await gate()) === 'stop') { stopped = true; break; }
        const cbAbort = await breaker.shouldAbort();
        if (cbAbort) { stopped = true; s.message = cbAbort; console.warn(`[SCRAPE] ${source.key}: ${cbAbort}`); pushRecent({ kind: 'earlystop', reason: cbAbort }); break; }
        try {
          ...
          if (!v.ok) {
            await quarantine({ sourceId: source.id, externalId: m.external_id, row: m.row, reasons: v.reasons, dedupeKey: dk, db });
            s.quarantined++;
            pageActivity++;
          ...
          if (!prev && dk) {
            const dupOf = await isDuplicate(dk, db);
            if (dupOf) {
              await quarantine({ sourceId: source.id, externalId: m.external_id, row: m.row, reasons: ['duplicate'], dedupeKey: dk, duplicateOf: dupOf, db });
              s.duplicates++;
              pageActivity++;
          ...
          // dualPrice only understands USD & PYG; any other currency (BOB, UYU, …)
          // it wrongly treats as USD. For those, trust the adapter's own USD figure
          // (derived from the listing's USD price) when it set one. PY (USD/PYG) is
          // byte-identical to before.
          const _cur = String(m.row.currency || '').toUpperCase();
          const _dualUsd = dualPrice(m.row.price, m.row.currency, fxRate).usd;
          const priceUsd = (_cur !== 'USD' && _cur !== 'PYG' && m.row.price_usd != null)
            ? m.row.price_usd
            : (_dualUsd ?? m.row.price_usd ?? null);
          if (!prev) {
            const [ins] = await insert('properties', [
              // short_code is the compact WhatsApp share slug (/s/<code>). PY-only:
              // the column exists on the PY DB, and stamping it here means scraped
              // listings get the short link on insert just like user-published ones.
          ...
          if (m.images.length) {
            if (cfg.skip_image_screen) {
              ...
              breaker.recordImages(m.images.length); // Vision billed for these images (runaway-cap accounting)
              const { kept, branded, rejected } = await screenPropertyImages(m.images.map((i) => i.source_url), source.key);
              ...
              for (const img of m.images) {
                ...
                if (brandedSet.has(img.source_url)) {
                  ...
                  const cleaned = await removeWatermark(img.source_url, cleanKey, source.key);
            ...
            if (screened.length) {
              const r = await mirrorImages(db, source, m.external_id, propertyId, screened);
              ...
              if (STAMP_LOGO) {
                try { const st = await stampListingImages(db, propertyId); if (st.featureUrl) featureUrl = st.featureUrl; }
...
export async function runScrape({ sourceKey, filters = {}, trigger = 'cron', country }) {
  const { runId, country: c } = await startRun({ sourceKey, filters, trigger, country });
  return runJob({ runId, country: c });
}
