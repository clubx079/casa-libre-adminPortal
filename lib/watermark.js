...
// opts:
//   opacity — 0..1, applied to the mascot glyph (and wordmark, if shown) (default 0.22, soft)
//   scale   — mascot width as a fraction of the base image width (default 0.72, large)
//   text    — show the "Casa Libre" wordmark under the mascot (default false)
export async function stampWatermark(inputBytes, opts = {}) {
  const { opacity = 0.22, scale = 0.72, text = false } = opts;
  ...
  // Centered horizontally. With the wordmark on, the mascot sits a touch above
  // center to leave room for text; with it off (default), the mascot is fully
  // centered so a large glyph fills the frame evenly.
  const mascotX = Math.round((width - mascotW) / 2);
  const mascotY = text
    ? Math.round(height / 2 - mascotH * 0.7)
    : Math.round((height - mascotH) / 2);
  ...
  if (text) {
    ...
    textEl = `<text x="${width / 2}" y="${textY}" text-anchor="middle"
