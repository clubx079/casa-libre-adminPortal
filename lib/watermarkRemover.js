...
async function detectWatermarkRegion(buffer, w, h, apiKey, source = 'watermark') {
  ...
  // Billed Vision request (LOGO + TEXT = 2 features). Log it so the usage monitor
  // sees watermark-detection spend — this call was previously uninstrumented and
  // the dashboard under-counted Casa Libre's real Vision billing.
  if (vr && vr.ok) void logApiCall({ api: 'vision', source, path: 'watermark-remove', calls: 2 });
...
export async function removeWatermark(sourceUrl, key, source = 'watermark') {
  ...
  try {
    ...
    const region = await detectWatermarkRegion(work, w, h, apiKey, source);
